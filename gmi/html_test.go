...
func BenchmarkToHTML(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		scanner := gmi.NewScanner(bytes.NewReader(input))

		converter, err := gmi.NewConverter(scanner, buf)
		if err != nil {
...
func expectHTML(t *testing.T, input, expected string) {
	...
	scanner := gmi.NewScanner(strings.NewReader(input))

	converter, err := gmi.NewConverter(scanner, &out)
	if err != nil {
